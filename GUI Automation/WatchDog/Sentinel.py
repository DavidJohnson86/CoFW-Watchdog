...
----------------------------------------------------------------------------------'''
    ...
    def watchdogTimer(self):
        ...
        if self.is_it_running(self.process_name) == False:
            ...
            GUI.CONSOLE_MESSAGE.put(
                "[CONSOLE]: Framework has been closed restarting it " + self.timestamp)
    ...
    def is_it_finished(self):
        if self.get_remaining_tests() is False:
            GUI.CONSOLE_MESSAGE.put("[CONSOLE]:No existing report has been found " + self.timestamp)
            return False
        if self.get_remaining_tests() is True:
            GUI.CONSOLE_MESSAGE.put(
                "[CONSOLE]:All The test was finished. Quitting..." +
                self.timestamp)
            sleep(10)
            os._exit(0)
    ...
    def runprocesses(self):
        ...
        '''
        GUI.CONSOLE_MESSAGE.put("[CONSOLE]:Parsing Latest Report " + self.timestamp)
        skipped_tests = self.get_remaining_tests()
        GUI.CONSOLE_MESSAGE.put("[CONSOLE]:Getting Latest Report " + self.timestamp)
        ...
        GUI.CONSOLE_MESSAGE.put("[CONSOLE]:TestList has been created Succesfully " + self.timestamp)
        #ReportHandler.Logger.logging('TestList has been created Succesfully')
